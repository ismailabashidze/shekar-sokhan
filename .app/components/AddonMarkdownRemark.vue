<script setup lang="ts">
  ...
  const props = withDefaults(
    ...
    }>(),
    {
      ...
      langs: () => ['html', 'vue', 'bash', 'python', 'dockerfile', 'json', 'yaml', 'markdown', 'diff'],
    },
  );
  ...
  watch(
    [() => props.source, processor],
    async ([source, _processor]) => {
...
<template>
  ...
  <BaseProse
    v-else
    :class="[proseSize, 'markdown', props.lines ? 'with-line-number' : '', props.fullwidth ? 'max-w-none' : '']"
  >
