...
export function useGoal() {
	...
	const getTherapyGoals = async (
		...
		try {
			...
			const result = await $pb
				.collection("suggestedDisordersToInvestigate")
				.getList(1, 50, {
					filter,
					sort: "-created",
				});
	...
	const createTherapyGoal = async (
		goalData: any,
	): Promise<TherapyGoal | null> => {
		try {
			const { user } = useUser();
	...
	const updateSubGoalStatus = async (
		...
		try {
			...
			const completedSubGoals = subGoals.filter(
				(sg: SubGoal) => sg.status === "completed",
			).length;
