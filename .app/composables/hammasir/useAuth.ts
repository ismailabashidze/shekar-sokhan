...
const register = async (data: RegisterDto): Promise<{ userId: string; emailVerificationRequired: boolean } | null> => {
  ...
  try {
    const result = await apiRequest<{ userId: string; emailVerificationRequired: boolean }>('/api/v1/auth/register', {
      method: 'POST',
...
const getGoogleOAuthURL = async (): Promise<GoogleOAuthUrlDto | null> => {
  try {
    const result = await apiRequest<GoogleOAuthUrlDto>('/api/v1/auth/oauth/google/url', {
      headers: {
        'X-Skip-Auth': 'true'
      }
    })
