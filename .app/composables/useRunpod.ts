...
export function useRunpod(pollingOptions: UseRunPodPollingOptions = {}) {
   ...
   */
  const runJob = async (input: any, options: RunPodOptions = {}, onChunk?: (chunk: any) => void): Promise<any> => {
    ...
    if (!targetEndpointId) {
      const err = new Error('Endpoint ID is required. Set it via options or endpointId ref.');
   ...
   */
  const getJobStatus = async (jobId: string, targetEndpointId?: string): Promise<RunPodStatus> => {
    try {
   ...
   */
  const startPolling = (targetEndpointId: string, jobId: string, onStatusChange?: (status: RunPodStatus) => void) => {
    if (pollTimer) {
  ...
  return {
    ...
    isPending: computed(
      () => currentJob.value && (currentJob.value.status === 'IN_QUEUE' || currentJob.value.status === 'IN_PROGRESS'),
    ),
