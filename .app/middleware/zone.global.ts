export default defineNuxtRouteMiddleware((to, from) => {
  console.log(
    '🔍 ZONE MIDDLEWARE - Navigating FROM:',
    from?.path,
    'TO:',
    to.path,
  )
  ...
  if (pathZone === '/admin') {
    const { role } = useUser()
    console.log(
      '🔍 ZONE MIDDLEWARE - Admin zone check - User role:',
      role.value,
    )
    if (role.value !== 'ADMIN') {
      console.log(
        '❌ ZONE MIDDLEWARE - Admin zone: User not admin, redirecting to zone restriction',
      )
  ...
  if (!hasAccess) {
    ...
    console.log(
      '❌ ZONE MIDDLEWARE - Zone: User does not have access, redirecting to zone restriction',
    )
