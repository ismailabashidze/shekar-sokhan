...
try {
  ...
  appVersion = packageJson.version || '3.6.0';
} catch (error) {
...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    runpodApiKey: process.env.RUNPOD_API_TOKEN || process.env.RUNPOD_API_KEY || '',
    ...
    public: {
      ...
      scihubApiBaseUrl: process.env.SCIHUB_API_BASE_URL || 'http://localhost:8000',
  ...
  vite: {
    build: {
      ...
      rollupOptions: {
        external: process.env.NODE_ENV === 'production' ? ['chromadb', '@anthropic-ai/sdk', '@gradio/client'] : [],
      },
    },
    // Optimize deps for faster builds
    optimizeDeps: {
      exclude: ['chromadb', '@anthropic-ai/sdk', '@gradio/client'],
    },
  ...
  pwa: {
    ...
    manifest: {
      name: 'ذهنا - پلتفرم درمان آنلاین',
      short_name: 'ذهنا',
      description: 'پلتفرم درمان آنلاین و مشاوره روانشناسی',
      theme_color: '#ffffff',
      icons: [
        {
          src: '/pwa-192x192.png',
