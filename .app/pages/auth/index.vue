  ...
  <div class="dark:bg-muted-800 flex min-h-screen bg-white">
      ...
      <div class="dark:bg-muted-800 relative mx-auto w-full max-w-sm bg-white">
        ...
        <div>
          ...
          <BaseParagraph size="sm" class="text-muted-400 mb-6">
            ورود با شبکه‌های اجتماعی یا اطلاعات کاربری
          </BaseParagraph>
          ...
          <div class="flex-100 mt-8 flex items-center">
            ...
            <span class="text-muted-600 dark:text-muted-300 px-4 font-sans font-light">
              یا
            </span>
            <hr class="border-muted-200 dark:border-muted-700 flex-auto border-t-2">
          </div>
            ...
            <div>
              <div class="space-y-4">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="email">
                  <BaseInput :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="email"
                    label="آدرس ایمیل" placeholder="آدرس ایمیل" autocomplete="email" :classes="{
                      input: 'h-12',
                ...
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="password">
                  <BaseInput :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="password"
                    label="رمز عبور" placeholder="رمز عبور" autocomplete="current-password" :classes="{
                      input: 'h-12',
                    }" @update:model-value="handleChange" @blur="handleBlur" />
                </Field>
              </div>
              ...
              <div class="mt-6 flex items-center justify-between">
                <Field v-slot="{ field, handleChange, handleBlur }" name="trustDevice">
                  <BaseCheckbox :model-value="field.value" :disabled="isSubmitting" shape="rounded"
                    label="مرا ۶۰ روز بخاطر بسپار" color="primary" @update:model-value="handleChange"
                    @blur="handleBlur" />
                </Field>
    ...
    <div class="bg-muted-100 dark:bg-muted-900 relative hidden w-0 flex-1 items-center justify-center lg:flex lg:w-3/5">
      <div class="mx-auto w-full max-w-4xl">
        <!--Media image-->
        <img class="mx-auto max-w-md" src="/img/illustrations/magician.svg" alt="" width="500" height="500">
      </div>
    </div>
  </div>
</template>
