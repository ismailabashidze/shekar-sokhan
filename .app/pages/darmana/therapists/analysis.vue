...
فقط متن پیام رو بنویس، بدون توضیح اضافی.`;
    ...
    const messages = [
      {
        role: 'system',
        content:
          'تو یک دستیار روانشناس متخصص هستی که در تجزیه و تحلیل جلسات درمانی و نوشتن پیام‌های هدفمند و شخصی‌سازی شده با در نظر گیری اطلاعات دموگرافیک و تحلیل‌های روانشناختی تخصص داری.',
      },
  ...
  const checkNotificationStatus = (step: any) => {
    ...
    const matchingNotification = userNotifications.value.find((notification) => {
      ...
      const messageMatch
        = notification.message.includes(step.suggestedMessage.substring(0, 50))
        || step.suggestedMessage.includes(notification.message.substring(0, 50));

      // Secondary match: title pattern
      const titleMatch
        = notification.title.includes('پیام مهربان')
        && (notification.title.includes(step.title.substring(0, 20))
        || step.title.includes(notification.title.substring(0, 20)));

      // Time-based match: scheduled time proximity (within 5 minutes)
      const timeMatch
        = notification.announce_time
  ...
  const processEnhancedNextSteps = async () => {
    ...
    if (hasGeneratedMessages) {
      ...
      enhancedNextSteps.value = analysisData.value.suggestedNextStepsForTherapistForNextSession.map(
        (step: any, index: number) => {
          ...
          const scheduledDate = step.scheduledDate
            ? new Date(step.scheduledDate)
            : new Date(Date.now() + schedule.hours * 60 * 60 * 1000);
    ...
    const deeperGoals
      = analysisData.value?.possibleDeeperGoalsOfPatient || 'بهبود وضعیت روانی و دستیابی به آرامش درونی';
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        stepsToProcess.push({
          ...originalStep,
          title:
            i >= originalSteps.length
              ? `${originalStep.title} (ادامه ${i - originalSteps.length + 2})`
              : originalStep.title,
        });
      }
    ...
    switch (status) {
      case 'not_checked':
        return {
          ...
          bgClass:
            'bg-gradient-to-r from-gray-500/15 to-slate-500/20 dark:from-gray-400/20 dark:to-slate-400/25 border border-gray-200/40 dark:border-gray-400/30',
      ...
      case 'not_created':
        return {
          ...
          bgClass:
            'bg-gradient-to-r from-blue-500/15 to-blue-600/20 dark:from-blue-400/20 dark:to-blue-500/25 border border-blue-200/30 dark:border-blue-400/20',
            ...
            <div class="text-center sm:text-right">
              ...
              <BaseParagraph size="lg" class="text-white opacity-80">
                <span class="mt-2 flex items-center justify-center gap-2 sm:justify-start">
                  ...
                  <BaseAvatar
                    :src="
                      getUserAvatarUrl(analysisData?.expand?.session?.expand?.user) || '/img/avatars/default-male.jpg'
                    "
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 backdrop-blur-sm">
                      ...
                      <span class="text-sm font-bold text-white">
                        {{
                          new Date(analysisData?.expand?.session?.start_time)
                            .toLocaleString('fa')
                            .split(', ')
                            .reverse()
                            .join(' - ')
                ...
                <div class="flex-1">
                  <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 backdrop-blur-sm">
                      ...
                      <span class="text-sm font-bold text-white">
                        {{
                          new Date(analysisData?.expand?.session?.end_time)
                            .toLocaleString('fa')
                            .split(', ')
                            .reverse()
                            .join(' - ')
                ...
                <div class="flex-1">
                  <div class="flex flex-col gap-1">
                    <span class="flex items-center gap-1 text-xs font-medium text-white/50">
                      <Icon
                        :name="
                          analysisData?.expand?.session?.status === 'done'
                    ...
                    >
                      <Icon
                        :name="
                          analysisData?.expand?.session?.status === 'done' ? 'ph:check-circle-fill' : 'ph:clock-fill'
                        "
                        class="size-4"
                        :class="{
              ...
              <BaseCard shape="curved" class="p-6">
                ...
                <div class="flex justify-between">
                  <BaseParagraph size="xs" class="text-muted-400 max-w-full">
                    ...
                    <span>
                      پیام های رد و بدل شده بین کاربر و مشاور به صورت محتوایی بررسی شده و خلاصه سازی گردیده است.
                    </span>
                    <NuxtLink to="#" class="text-primary-500 underline-offset-4 hover:underline">
                      اطلاعات بیشتر
                    </NuxtLink>
                  </BaseParagraph>
                </div>
                ...
                <div class="grid w-full gap-6">
                  ...
                  <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="bg-muted-800 dark:bg-muted-900 relative h-[400px] overflow-hidden rounded-2xl p-6">
                      <div
                        class="bg-primary-500/10 pointer-events-none absolute right-0 top-0 size-32 rounded-bl-[6rem]"
                      />
                      <div
                        class="bg-primary-500/5 pointer-events-none absolute bottom-0 left-0 size-24 rounded-tr-[4rem]"
                      />
                      <div class="relative z-10">
                          ...
                          <div class="text-center">
                            <div class="flex items-baseline justify-center gap-1">
                              ...
                              <span class="text-primary-500 text-7xl font-bold leading-none tracking-tight">
                                {{
                                  100 -
                                    (analysisData.negativeScoresList?.reduce(
                        ...
                        <div class="grid grid-cols-2 gap-4">
                          <div class="bg-muted-900/50 rounded-xl p-4">
                            ...
                            <div class="flex items-baseline justify-center gap-1">
                              <span class="text-2xl font-semibold text-white">
                                {{ analysisData?.expand?.session?.total_time_passed }}
                              </span>
                          ...
                          <div class="bg-muted-900/50 rounded-xl p-4">
                            ...
                            <div class="flex items-baseline justify-center gap-1">
                              <span class="text-2xl font-semibold text-white">
                                {{ analysisData?.expand?.session?.count_of_total_messages }}
                              </span>
                  ...
                  <div class="mt-6 grid grid-cols-1 gap-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                      ...
                      <div
                        class="bg-muted-100 dark:bg-muted-800/50 border-muted-200 dark:border-muted-700 h-full rounded-xl border p-5"
                      >
                        ...
                        <div class="grid gap-3">
                          ...
                          >
                            <div class="flex items-start gap-3">
                              <div
                                class="bg-danger-500/10 dark:bg-danger-500/20 flex size-8 shrink-0 items-center justify-center rounded-lg"
                              >
                      ...
                      >
                        ...
                        <div class="grid gap-3">
                          ...
                          >
                            <div class="flex items-start gap-3">
                              <div
                                class="bg-success-500/10 dark:bg-success-500/20 flex size-8 shrink-0 items-center justify-center rounded-lg"
                              >
                          ...
                          <div
                            v-if="
                              !analysisData.psychotherapistEvaluationScorePositiveBehavior ||
                                analysisData.psychotherapistEvaluationScorePositiveBehavior.length === 0
                            "
                            class="bg-muted-100 dark:bg-muted-700/30 border-muted-200 dark:border-muted-700 rounded-xl border p-4 text-center"
                          >
                    ...
                    >
                      ...
                      <div class="grid gap-3">
                        ...
                        >
                          <div class="flex items-start gap-3">
                            <div
                              class="bg-primary-500/10 dark:bg-primary-500/20 flex size-8 shrink-0 items-center justify-center rounded-lg"
                            >
                        ...
                        <div
                          v-if="
                            !analysisData.psychotherapistEvaluationScoreSuggestionsToImprove ||
                              analysisData.psychotherapistEvaluationScoreSuggestionsToImprove.length === 0
                          "
                          class="bg-muted-100 dark:bg-muted-700/30 border-muted-200 dark:border-muted-700 rounded-xl border p-4 text-center"
                        >
            ...
            <div class="mt-5 grid grid-cols-12 gap-6">
                ...
                <BaseCard shape="curved" class="p-6">
                  ...
                  <div class="flex justify-between">
                    <BaseParagraph size="xs" class="text-muted-400 max-w-full">
                      <Icon name="ph:question-duotone" class="size-4" />
                      <span>
                        اطلاعات دموگرافیک کاربر که می تواند توسط خود کاربر اظهار شده یا در جریان مصاحبه و گفت و گو
                        استخراج گردد.
                      </span>
                      <NuxtLink to="#" class="text-primary-500 underline-offset-4 hover:underline">
                        اطلاعات بیشتر
                    ...
                    <div class="grid grid-cols-12 gap-4">
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.firstName"
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.lastName"
                            class="bg-success-500/10 text-success-500 rounded-full px-2 py-0.5 text-xs"
                          >
                            موجود
                          </span>
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.age"
                            class="bg-success-500/10 text-success-500 rounded-full px-2 py-0.5 text-xs"
                          >
                            موجود
                          </span>
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.gender"
                            class="bg-success-500/10 text-success-500 rounded-full px-2 py-0.5 text-xs"
                          >
                            موجود
                          </span>
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.education"
                            class="bg-success-500/10 text-success-500 rounded-full px-2 py-0.5 text-xs"
                          >
                            موجود
                          </span>
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.occupation"
                            class="bg-success-500/10 text-success-500 rounded-full px-2 py-0.5 text-xs"
                          >
                            موجود
                          </span>
                      ...
                      <div class="col-span-12">
                        <div class="mb-1 flex items-center justify-between">
                          ...
                          <span
                            v-if="analysisData.demographicData?.maritalStatus"
                            class="bg-success-500/10 text-success-500 rounded-full px-2 py-0.5 text-xs"
                          >
                            موجود
                          </span>
                  ...
                  >
                    ...
                    >
                      <Icon name="ph:note-pencil-duotone" class="text-muted-400 mb-2 size-12" />
                      <p class="text-muted-500 dark:text-muted-400">
              ...
              <div class="col-span-4">
                <BaseCard class="p-6">
                  ...
                  <BaseParagraph size="xs" class="text-muted-400 max-w-full">
                    <Icon name="ph:question-duotone" class="size-4" />
                    <span>میزان مشارکت و اعتماد ارزیابی شده از پیام های کاربر</span>
                  </BaseParagraph>
                  <div class="mt-[80px] flex flex-col gap-6 md:flex-row md:items-end">
                  ...
                  <BaseCard shape="curved" class="p-6">
                    ...
                    <div class="flex justify-between">
                      <BaseParagraph size="xs" class="text-muted-400 max-w-full">
                        ...
                        <span>عوامل خطر شناسایی شده که نیاز به توجه ویژه دارند</span>
                      </BaseParagraph>
                    </div>
                        ...
                        >
                          ...
                          >
                            <div class="flex w-full items-start gap-3">
                              <div
                                class="bg-warning-500/10 dark:bg-warning-500/20 flex size-8 shrink-0 items-center justify-center rounded-lg"
                              >
                                <Icon name="ph:warning-circle-duotone" class="text-warning-500 size-5" />
                              </div>
                              <div class="flex-1">
                                <BaseHeading
                                  as="h4"
                    ...
                    <div v-else class="mt-6 text-center">
                      <Icon name="ph:shield-check-duotone" class="text-muted-400 mb-2 size-12" />
                ...
                <BaseCard shape="curved" class="p-6">
                  ...
                  <div class="flex justify-between">
                    <BaseParagraph size="xs" class="text-muted-400 max-w-full">
                      ...
                      <span>
                        تحلیل روانی پیام های کاربر نشان می دهد که وی در چه وضعیتی از لحاظ ارتباط روانی با هوش مصنوعی
                        است.
                      </span>
                    </BaseParagraph>
                  </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                      ...
                      >
                        <BaseCard
                          ...
                          :class="{
                            'border-success-500 dark:border-success-400 hover:bg-success-50 dark:hover:bg-success-500/10':
                              mechanism.confidence === 'very_high',
                            'border-info-500 dark:border-info-400 hover:bg-info-50 dark:hover:bg-info-500/10':
                              mechanism.confidence === 'high',
                            'border-warning-500 dark:border-warning-400 hover:bg-warning-50 dark:hover:bg-warning-500/10':
                              mechanism.confidence === 'low',
                            'border-muted-300 dark:border-muted-700 hover:bg-muted-50 dark:hover:bg-muted-800':
                              mechanism.confidence === 'very_low',
                          }"
                        >
                          <div class="flex w-full items-start gap-3">
                            <div
                            ...
                            >
                              <Icon
                                :name="
                                  mechanism.name.toLowerCase().includes('انکار')
                                    ? 'ph:prohibit-duotone'
                                    : mechanism.name.toLowerCase().includes('فرافکنی')
                                      ? 'ph:arrows-out-duotone'
                                      : mechanism.name.toLowerCase().includes('درون‌فکنی')
                                        ? 'ph:arrow-fat-lines-down-duotone'
                                        : mechanism.name.toLowerCase().includes('جابجایی')
                                          ? 'ph:arrows-left-right-duotone'
                                          : mechanism.name.toLowerCase().includes('واکنش')
                                            ? 'ph:arrows-out-duotone'
                                            : mechanism.name.toLowerCase().includes('دلیل')
                                              ? 'ph:brain-duotone'
                                              : 'ph:shield-duotone'
                                "
                                class="size-5"
                            ...
                            <div class="flex-1">
                              ...
                              <BaseText
                                size="xs"
                                class="text-muted-400 line-clamp-2 transition-all duration-300 group-hover:line-clamp-none"
                              >
                                {{ mechanism.evidence }}
                              </BaseText>
                            </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                      ...
                      >
                        <BaseCard
                          ...
                          :class="{
                            'border-success-500 dark:border-success-400 hover:bg-success-50 dark:hover:bg-success-500/10':
                              schema.confidence === 'very_high',
                            'border-info-500 dark:border-info-400 hover:bg-info-50 dark:hover:bg-info-500/10':
                              schema.confidence === 'high',
                            'border-warning-500 dark:border-warning-400 hover:bg-warning-50 dark:hover:bg-warning-500/10':
                              schema.confidence === 'low',
                            'border-muted-300 dark:border-muted-700 hover:bg-muted-50 dark:hover:bg-muted-800':
                              schema.confidence === 'very_low',
                          }"
                        >
                          <div class="flex w-full items-start gap-3">
                            <div
                            ...
                            >
                              <Icon
                                :name="
                                  schema.name.toLowerCase().includes('رهاشدگی')
                                    ? 'ph:person-simple-walk-duotone'
                                    : schema.name.toLowerCase().includes('بی‌اعتمادی')
                                      ? 'ph:warning-circle-duotone'
                                      : schema.name.toLowerCase().includes('نقص')
                                        ? 'ph:puzzle-piece-duotone'
                                        : schema.name.toLowerCase().includes('انزوا')
                                          ? 'ph:user-circle-minus-duotone'
                                          : schema.name.toLowerCase().includes('شکست')
                                            ? 'ph:x-circle-duotone'
                                            : schema.name.toLowerCase().includes('وابستگی')
                                              ? 'ph:link-duotone'
                ...
                <BaseCard shape="curved" class="p-6">
                  ...
                  <div class="flex justify-between">
                    <BaseParagraph size="xs" class="text-muted-400 max-w-full">
                      ...
                      <span>تحلیل اهداف و انگیزه‌های عمیق‌تر مراجع که ممکن است به صورت صریح بیان نشده باشند</span>
                    </BaseParagraph>
                  </div>
                  <div v-if="analysisData.possibleDeeperGoalsOfPatient" class="mt-5">
                    <div
                      class="bg-muted-100 dark:bg-muted-800/50 border-primary-100 dark:border-primary-500/20 rounded-xl border p-4 transition-all hover:shadow-md"
                    >
                      <div class="mb-3 flex items-center gap-3">
                        <div class="bg-primary-500/10 dark:bg-primary-500/20 rounded-lg p-2">
              ...
              <div class="col-span-12 mb-8">
                <BaseCard
                  shape="curved"
                  class="border-primary-100/30 dark:border-primary-500/20 overflow-hidden border-2 p-0"
                >
                  <!-- Header Section with Gradient -->
                  <div
                    class="from-primary-500/10 via-primary-400/5 dark:from-primary-600/20 dark:via-primary-500/10 border-primary-100/30 dark:border-primary-500/20 border-b bg-gradient-to-r to-orange-400/10 p-6 dark:to-orange-500/15"
                  >
                    <div class="mb-2 flex items-center justify-between">
                      <div class="flex items-center gap-4">
                        <div
                          class="from-primary-500 dark:from-primary-400 flex size-14 items-center justify-center rounded-2xl bg-gradient-to-br to-orange-500 shadow-lg dark:to-orange-400"
                        >
                        ...
                        <div>
                          ...
                          <BaseParagraph size="sm" class="text-muted-600 dark:text-muted-300">
                            ...
                            <span>پیشنهادات مهربانانه و زمان‌بندی مناسب برای ادامه ارتباط با مراجع</span>
                          </BaseParagraph>
                        </div>
                      </div>
                  ...
                  <div class="p-6">
                    ...
                    <div v-if="notificationsCreated || enhancedNextSteps.length > 0" class="mb-6">
                      <div
                        class="rounded-xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 p-4 dark:border-emerald-700 dark:from-emerald-900/20 dark:to-teal-900/20"
                      >
                      ...
                      <div class="grid gap-6">
                        ...
                        >
                          ...
                          >
                            <!-- Gradient Background -->
                            <div
                              class="from-primary-50/50 to-primary-50/30 dark:from-primary-900/20 dark:via-muted-900 dark:to-primary-800/10 absolute inset-0 bg-gradient-to-br via-white opacity-80 transition-opacity duration-500 group-hover:opacity-100"
                            />
                            ...
                            <div class="relative z-10 p-6">
                              ...
                              <div class="mb-6 flex items-start justify-between">
                                <div class="flex items-start gap-4">
                                  <div
                                    class="from-primary-500 to-primary-600 dark:from-primary-400 dark:to-primary-500 flex size-12 items-center justify-center rounded-xl bg-gradient-to-br shadow-lg transition-transform duration-300 group-hover:rotate-3 group-hover:scale-110"
                                  >
                              ...
                              <div
                                class="from-muted-50/80 to-muted-100/60 dark:from-muted-800/40 dark:to-muted-800/60 dark:border-muted-700/30 rounded-2xl border border-white/50 bg-gradient-to-r p-5 backdrop-blur-sm"
                              >
                                <div class="mb-4 flex items-center justify-between">
                                  <div class="flex items-center gap-3">
                                    <div
                                      class="rounded-lg bg-gradient-to-r from-pink-500/10 to-rose-500/10 p-2 dark:from-pink-400/20 dark:to-rose-400/20"
                                    >
                                ...
                                <div
                                  class="dark:border-muted-700 dark:bg-muted-900/80 rounded-xl border border-white bg-white/80 p-5 shadow-sm backdrop-blur-sm"
                                >
                                  ...
                                  <div class="flex items-center gap-2">
                                    ...
                                    >
                                      {{
                                        checkNotificationStatus(step).notificationExists
                                          ? checkNotificationStatus(step).actualStatus === 'scheduled'
                                            ? `این پیام در صف ارسال قرار دارد و در ${new Date(
                                              checkNotificationStatus(step).announceTime,
                    ...
                    <div v-else class="mt-6 text-center">
                      <Icon name="ph:clipboard-text-duotone" class="text-muted-400 mb-2 size-12" />
                      <BaseText size="sm" class="text-muted-400">
