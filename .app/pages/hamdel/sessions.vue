<script setup lang="ts">
  ...
  const checkAccess = () => {
    const queryUserId = Array.isArray(route.query.userId) ? route.query.userId[0] : route.query.userId;
  ...
  const filteredSessions = computed(() => {
    ...
    return sessions.value.filter((session) => {
      ...
      return therapistName.toLowerCase().includes(searchTerm) || session.id.toLowerCase().includes(searchTerm);
    });
  });
    ...
    <TairoContentWrapper>
      ...
      <div v-else-if="!loading && filteredSessions.length === 0">
        <BasePlaceholderPage
          ...
          :subtitle="
            statusFilter === 'all'
              ? 'هیچ جلسه‌ای در حال انجام یا تکمیل شده یافت نشد. لطفا یک جلسه جدید شروع کنید.'
              : 'هیچ جلسه‌ای با این مشخصات پیدا نشد. لطفا معیارهای جستجوی خود را تغییر دهید یا یک جلسه جدید شروع کنید.'
          "
        >
          <template #image>
            <img
          ...
          >
            <div class="bg-muted-50 dark:bg-muted-800/30 border-muted-200 dark:border-muted-700 border-b p-4">
              <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="flex items-center">
                  <BaseAvatar
                    :src="
                      session.expand?.therapist?.avatar
                        ? `https://pocket.zehna.ir/api/files/therapists/${session.expand.therapist.id}/${session.expand.therapist.avatar}`
            ...
            <div class="flex-1 p-4">
              ...
              <div class="grid h-full grid-cols-1 gap-4">
                ...
                <div
                  v-if="session.expand?.session_analysis_for_system"
                  class="bg-muted-50 dark:bg-muted-900/50 rounded-lg p-4"
                >
                  ...
                  <div class="space-y-3">
                    ...
                    <!-- Trust Level -->
                    <div
                      v-if="session.expand.session_analysis_for_system.finalTrustAndOppennessOfUser"
                      class="flex items-center justify-between"
                    >
                      <span class="text-muted-600 dark:text-muted-400 text-xs">سطح اعتماد:</span>
                      <BaseTag
                        :color="
                          getTrustLevelColor(session.expand.session_analysis_for_system.finalTrustAndOppennessOfUser)
                        "
                        shape="curved"
                        size="sm"
                    ...
                    >
                      ...
                      <div class="space-y-1">
                        ...
                        >
                          <span class="text-muted-700 dark:text-muted-300 text-xs font-medium">
                            {{ headline.title }}
                          </span>
