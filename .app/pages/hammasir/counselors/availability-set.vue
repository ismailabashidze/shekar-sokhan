...
const loadAvailability = async () => {
  try {
    ...
    if (counselorProfile.value?.availability) {
      availability.value = {
        ...counselorProfile.value.availability,
        weeklySchedule: counselorProfile.value.availability.weeklySchedule || []
      }
    } else {
...
const getDaySchedule = (dayEn: string) => {
  return availability.value.weeklySchedule.find(d => d.dayOfWeek === dayEn) || {
    dayOfWeek: dayEn,
    isEnabled: false,
    timeSlots: []
  }
...
const saveAvailability = async () => {
  ...
  try {
    ...
    await updateAvailability(availability.value)
