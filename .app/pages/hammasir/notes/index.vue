...
const handleAddNote = async (clientId: string, content: string) => {
  ...
  const result = await addClientNote(clientId, content)
  if (result) {
    // Reset form
...
<template>
  <div class="py-6">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              مدیریت یادداشت‌های کاربران مشاوره‌ای
            </p>
          </div>
          
          <div class="mt-4 flex space-x-3 md:mt-0 space-x-reverse">
            <NuxtLink
              to="/hammasir/notes/dashboard"
            ...
            >
              <Icon name="ph:chart-line" class="ml-2 h-4 w-4" />
              <span>داشبورد</span>
            </NuxtLink>
      ...
      <!-- Empty State -->
      <div v-else-if="!selectedClientId" class="mt-12 rounded-lg border-2 border-dashed border-gray-300 p-12 text-center dark:border-gray-700">
        <Icon name="ph:note" class="mx-auto h-12 w-12 text-gray-400" />
        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">
          هیچ کاربری انتخاب نشده است
        </h3>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          برای مشاهده یادداشت‌ها، شناسه کاربر را وارد کنید.
        </p>
      </div>
      
      <div v-else-if="filteredNotes.length === 0 && selectedClientId" class="mt-12 rounded-lg border-2 border-dashed border-gray-300 p-12 text-center dark:border-gray-700">
        <Icon name="ph:note" class="mx-auto h-12 w-12 text-gray-400" />
        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">
          یادداشتی یافت نشد
        </h3>
      ...
      <div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block transform overflow-hidden rounded-lg bg-white text-right align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle dark:bg-gray-800">
          <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              <div class="mt-3 text-center sm:mt-0 sm:mr-4 sm:text-right">
                ...
                <div class="mt-4">
                  <form @submit.prevent="handleAddNote(selectedClientId, newNoteContent)" class="space-y-4">
                    <div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 dark:bg-gray-700">
            <button
              type="button"
              :disabled="isNotesCreating || !selectedClientId || !newNoteContent"
              @click="handleAddNote(selectedClientId, newNoteContent)"
