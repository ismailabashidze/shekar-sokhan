...
import { LLMMessage } from '~/composables/llm'
...
onMounted(async () => {
  ...
  msgs.value.map((m) => {
    if (m.sender === 'user') {
      let label = 0
      const temp = m.evaluations.suicide_risk_labeler
        .toLowerCase()
        .replace(' ', '')
        .replace('.', '')
        .replace('\n', '')
...
const generateUserSummary = async () => {
  ...
  const userMsgs = (convertedMessages('BackendMessage') as BackendMessage[])
    .map((item: BackendMessage) => (item.sender === 'user' ? item : undefined))
    .filter(Boolean)
  ...
  for (let i = 0; i < userMsgs.length; i++) {
    ...
    while (retryCounter < maxRetries) {
      ...
      if (data.value.result.length > 30) {
        ...
        if (analyze === 'suicide_risk_labeler') {
          let label = 0
          const temp = m.evaluations.suicide_risk_labeler
            .toLowerCase()
            .replace(' ', '')
                ...
                <BaseCard shape="curved" class="p-6">
                  ...
                  <div class="flex justify-between">
                    ...
                    <BaseButton
                      ...
                      :loading="isLoading"
                      :disabled="userSum.length != 0 || isLoading"
                      >ایجاد</BaseButton
                    >
                  </div>
