...
const previewSummary = computed(() => {
  ...
  return [
    `Mode: ${state.multiMsgMode === 'multi_short' ? 'Multiple short messages' : 'Single message'}`,
    `Length: ${state.lengthPref}`,
    `Emoji: ${emojiMap[state.emojiLevel]}`,
    `Tone: ${state.tone}`,
    `Kindness: ${state.kindness}`,
    `Creativity: ${state.creativity}`,
    `Domain strictness: ${state.domainStrictness}`,
    `Style: ${state.languageStyle}`,
    `Disclaimers: ${state.disclaimers}`,
    `Profanity: ${state.profanity}`,
    `Formatting: ${state.formatting}`,
    `Reply speed: ${state.replySpeedMs}ms`
  ].join(' • ')
})
  ...
  <div class="p-6">
    <div class="max-w-5xl mx-auto space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Message mode</h3>
          </div>
          <div class="flex gap-3">
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.multiMsgMode==='single' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.multiMsgMode='single'">Single</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.multiMsgMode==='multi_short' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.multiMsgMode='multi_short'">Multiple short</button>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Message length</h3>
          </div>
          <div class="flex gap-3 flex-wrap">
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.lengthPref==='very_short' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.lengthPref='very_short'">Very short</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.lengthPref==='short' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.lengthPref='short'">Short</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.lengthPref==='long' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.lengthPref='long'">Long</button>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Emoji usage</h3>
          </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Tone</h3>
          </div>
          <div class="flex gap-3">
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.tone==='formal' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.tone='formal'">Formal</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.tone==='informal' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.tone='informal'">Informal</button>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Kindness</h3>
          </div>
          <div class="flex gap-3 flex-wrap">
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.kindness==='very_kind' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.kindness='very_kind'">Very kind</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.kindness==='kind' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.kindness='kind'">Kind</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.kindness==='not_kind' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.kindness='not_kind'">Not kind at all</button>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          ...
          <div class="text-xs text-muted-600 mt-1">
            {{ state.creativity === 0 ? 'Deterministic' : state.creativity === 1 ? 'Balanced' : 'Creative' }}
          </div>
        </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Domain strictness</h3>
          </div>
          <div class="flex gap-3">
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.domainStrictness==='strict' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.domainStrictness='strict'">Strict</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.domainStrictness==='balanced' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.domainStrictness='balanced'">Balanced</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.domainStrictness==='loose' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.domainStrictness='loose'">Loose</button>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Language style</h3>
          </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Disclaimers</h3>
          </div>
        ...
        <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-5">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-medium">Profanity</h3>
          </div>
          <div class="flex gap-3">
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.profanity==='block' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.profanity='block'">Block</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.profanity==='soften' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.profanity='soften'">Soften</button>
            <button :class="['px-3 py-2 rounded-lg text-sm border', state.profanity==='allow' ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-muted-700 border-muted-300']" @click="state.profanity='allow'">Allow</button>
          </div>
        </div>
      ...
      <div class="rounded-xl border border-muted-200 bg-white shadow-sm p-4 flex items-center justify-between">
        ...
        <NuxtLink to="/darmana/therapists/messaging" class="btn bg-primary-600 text-white rounded-lg px-4 py-2 hover:bg-primary-700">
...
.form-select {
  @apply rounded-lg border border-muted-300 bg-white px-3 py-2 text-sm text-muted-800 focus:outline-none focus:ring-2 focus:ring-primary-500/50 w-full;
}
</style>
